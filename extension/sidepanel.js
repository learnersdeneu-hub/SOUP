// SOUP Companion — side panel UI.
// Vanilla JS by design: this keeps the extension loadable unpacked with no
// build step. It re-renders the whole panel body on every state change,
// which is simple, correct, and fast enough for a panel this size.
...
function renderConnect() {
  return el(`
    <div class="sc-card">
      ...
      <a class="sc-link" style="font-size:11.5px;" href="${API_BASE}/companion/connect" data-action="open-connect-page" target="_blank" rel="noreferrer">Open My SOUP → SOUP Companion</a>
    </div>
  `);
}
...
function renderDocumentsList(documents) {
  if (!documents.length) return `<p class="sc-note" style="margin-top:8px;">No documents in your SOUP vault yet. <a class="sc-link" href="${API_BASE}/documents" target="_blank" rel="noreferrer">Upload one</a>.</p>`;
  ...
  return el(`
    <div>
      ...
      <div class="sc-card">
        ...
        <p class="sc-note">Noodles already knows you're on ${escapeHtml(pageNote)}. Opens in a new SOUP tab so you keep your progress here.</p>
...
async function doPair() {
  ...
  try {
    const body = await apiFetch("/api/companion/pair", { method: "POST", body: JSON.stringify({ code, deviceLabel: navigator.userAgent.slice(0, 60) }) });
...
async function runScan(quiet) {
  ...
  try {
    ...
    const scan = await sendToActiveTab({ type: "SOUP_SCAN" });
    ...
    state.documentsOpen = state.documentsOpen || false;
    const counts = fieldCounts();
...
async function startFill() {
  ...
  const response = await sendToActiveTab({ type: "SOUP_FILL", assignments: plan.map((p) => ({ fieldId: p.fieldId, value: p.value })) });
...
async function saveAnswer(fieldId) {
  ...
  if (!value) return;
  await sendToActiveTab({ type: "SOUP_FILL", assignments: [{ fieldId, value }] });
...
async function applySoupValue(fieldId) {
  const entry = state.resolved.find((r) => r.field.id === fieldId);
  if (!entry) return;
  await sendToActiveTab({ type: "SOUP_FILL", assignments: [{ fieldId, value: entry.answer.value }] });
  entry.status = "ready";
...
function submitAsk() {
  ...
  chrome.tabs.create({ url: `${API_BASE}/counselor?intent=universities&prompt=${encodeURIComponent(prompt)}` });
}
...
chrome.runtime.onMessage.addListener((message) => {
  if (message && message.type === "SOUP_TAB_CHANGED" && state.token) runScanQuiet();
});
