...
import { ApplicantInfoForm } from "@/components/applications/ApplicantInfoForm";
import { RequirementUploadButton } from "@/components/applications/RequirementUploadButton";
...
export default async function ApplicationDetailPage({ params }: { params: { id: string } }) {
  ...
  return <div className="min-h-screen bg-paper"><Header signedIn/><main className="mx-auto max-w-5xl px-5 py-8 sm:px-8">
    ...
    <section className="mt-5 rounded-2xl border border-hair bg-white p-5 sm:p-6">
      ...
      {!applicantInfoComplete && <div className="mt-3 rounded-xl bg-paper p-3 text-[11px] leading-5 text-mute">Still needed: {missingCore.join(" · ")}</div>}
      <ApplicantInfoForm prefill={{
        fullName: application.profile.user.fullName || "",
        dateOfBirth: application.profile.user.dateOfBirth ? application.profile.user.dateOfBirth.toISOString().slice(0, 10) : "",
        nationality: application.profile.user.nationality || "",
        currentCountry: application.profile.user.currentCountry || "",
        academicBackgroundSummary: application.profile.studentCase?.academicBackgroundSummary || "",
      }}/>
