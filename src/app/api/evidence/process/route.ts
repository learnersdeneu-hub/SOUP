...
export async function POST(request: Request) {
  ...
  // Routine confirmation, not urgent — gated by the general (not "important")
  // email preference, and never allowed to affect the already-successful upload.
  if (shouldSendStudentEmail(profile, false)) await sendTransactionalEmail({
    to: profile.user.email,
    subject: "Document received",
    html: `<p>Hello ${escapeHtml(profile.user.fullName)},</p><p>SOUP received and read <strong>${escapeHtml(file.name)}</strong>. It is saved in your document vault and ready for the next step.</p>`,
    idempotencyKey: `document-received-${document.id}`,
  }).catch((error) => logServerError("SOUP_DOCUMENT_RECEIVED_EMAIL_ERROR", error));
