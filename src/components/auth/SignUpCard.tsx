...
// lands on /auth/callback, which calls ensureUserAndProfile; nothing here
...
export function SignUpCard({ next, subtitle }: { next: string; subtitle: string }) {
  ...
  if (step === "sent") {
    return <CheckEmailStep email={email} fullName={fullName} institutionName={institutionName} next={next} onChangeEmail={() => setStep("details")} />;
  }
